 ...
 */
class SharedStreamDispatcher extends EventEmitter {
    constructor(client, player, stream, sd, streamOptions) {
        super();
        this.client = client;
        this.player = player;
        this.stream = stream;
    ...
    _sendBuffer(vc, opusBuffer, sequence, timestamp) {
        let repeats = this.passes;
        const packet = this._createPacket(vc, sequence, timestamp, opusBuffer);
        while (repeats--) {
            vc.sockets.udp.send(packet)
                .catch(e => this.emit('debug', `failed to send a packet ${e}`));
        }
    }

    _createPacket(vc, sequence, timestamp, buffer) {
        ...
        packetBuffer.writeUIntBE(vc.authentication.ssrc, 8, 4);

        packetBuffer.copy(nonce, 0, 0, 12);
        buffer = NaCl.secretbox(buffer, nonce, vc.authentication.secretKey.key);
    ...
    _send() {
        try {
            ...
            if (this.paused) {
                ...
                this.client.setTimeout(() => this._send(), data.length * 10);
                return;
            }
            ...
            let opusBuffer = this.player.opusEncoder.encode(buffer);
            for (let i = 0; i < this.player.voiceConnections.length; i++) {
                let vc = this.player.voiceConnections[i];
                this._sendBuffer(vc, opusBuffer, data.sequence, data.timestamp);
            }

            const nextTime = data.length + (data.startTime + data.pausedTime + (data.count * data.length) - Date.now());
            this.client.setTimeout(() => this._send(), nextTime);
        } catch (e) {
...
module.exports = SharedStreamDispatcher;
